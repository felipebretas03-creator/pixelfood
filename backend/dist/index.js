...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: restaurant.id, role: 'owner' }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
...
app.post('/api/auth/login', async (req, res) => {
    ...
    const isValid = await bcryptjs_1.default.compare(password, restaurant.password);
    if (!isValid) {
    ...
    const token = jsonwebtoken_1.default.sign({ id: restaurant.id, role: 'owner', isMaster: restaurant.isMaster }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
    res.json({
        token,
        id: restaurant.id,
...
const tenantMiddleware = async (req, res, next) => {
    ...
    const id = req.headers['x-restaurant-id'];
    let restaurant;
    if (id) {
...
app.post('/api/auth/customer/register', async (req, res) => {
    ...
    try {
        ...
        if (existing)
            return res.status(400).json({ error: 'E-mail já cadastrado neste restaurante' });
        const hashedPassword = await bcryptjs_1.default.hash(password, 10);
        ...
        const token = jsonwebtoken_1.default.sign({ id: customer.id, role: 'customer', restaurantId: req.restaurantId }, process.env.JWT_SECRET || 'secret', { expiresIn: '30d' });
...
app.put('/api/settings', ownerMiddleware, async (req, res) => {
    try {
        const { storeName, primaryColor, deliveryType, deliveryFee, isOpen, mpAccessToken, mpPublicKey, acceptPix, acceptCreditCardOnline, acceptCardMachine, acceptCash } = req.body;
        let settings = await prisma.settings.findUnique({
            where: { restaurantId: req.restaurantId }
        });
        if (!settings) {
...
app.delete('/api/neighborhoods/:id', ownerMiddleware, async (req, res) => {
    try {
        const neighborhood = await prisma.neighborhoodFee.findUnique({ where: { id: req.params.id } });
...
app.post('/api/customer/addresses', async (req, res) => {
    ...
    try {
        ...
        if (!street || !number)
            return res.status(400).json({ error: 'Rua e número são obrigatórios' });
        if (isDefault) {
...
app.post('/api/checkout/pix', async (req, res) => {
    try {
        ...
        const client = new mercadopago_1.MercadoPagoConfig({ accessToken: settings.mpAccessToken });
    ...
    try {
        ...
        orders.forEach(order => {
            ...
            if (chartData[mappedIndex])
                chartData[mappedIndex].vendas += order.total;
        });
        res.json({
            totalRevenue,
            totalOrders,
